...
pub(super) struct CachePages {
    keys: BTreeMap<CacheKey, usize>,
    slots: Vec<Slot>,
    oldest: usize,
    newest: usize,
}

impl Default for CachePages {
    fn default() -> Self {
        Self {
            keys: BTreeMap::new(),
            slots: Vec::new(),
...
impl CachePages {
    ...
    pub fn page_mut(&mut self, index: usize) -> Option<&mut CachedPage> {
        self.slots.get_mut(index).map(|slot| &mut slot.page)
    }

    pub fn touch(&mut self, key: &CacheKey) -> Option<usize> {
    ...
    pub fn insert(
        ...
        let index = if evicted {
            ...
            self.unlink(index);
            // Dropping the old slot zeroizes its plaintext page; no stale layout survives.
...
impl CachePages {
    ...
    pub fn resident_slot(&self, key: &CacheKey) -> Option<usize> {
    ...
    pub fn values(&self) -> impl Iterator<Item = &CachedPage> {
        self.slots.iter().map(|slot| &slot.page)
    }
    pub fn ordered_keys(&self) -> impl Iterator<Item = CacheKey> + '_ {
    ...
    pub fn inline_allowance() -> usize {
        2 * size_of::<Slot>() + 2 * size_of::<(CacheKey, usize)>()
    }
}
