...
D0264 is committed/pushed as `ede012f`. Its one supervised 96/128/32 MiB observation completed
...
percentile and RSS fields, the complete report exactly matches D0263 at SHA-256
...
The round took 542,925 ms, 0.25% below D0263. Retained all-class successful p99 was **2.634285 /
87.525892 / 61.595940 / 368.916668 ms**, changing by -4.79%, -4.09%, -2.49% and -3.39%. This single
sequential uncontrolled observation cannot establish causality; four-hop remains 1.48 times the
unchanged 250 ms target, so no target, T-20, M1 or qualification gate passes.

Process peak RSS was 265,820 KiB; wall/user/system were 720.28/664.74/53.84 seconds with zero
process swaps. Inputs and the D0264 binary stayed pinned. Shared memory/swap peaks and
soft-limit/maximum/OOM/socket-memory/CPU-throttle counters were unchanged. Complete report:
`docs/evidence/borrowed-singleton-selected-map-sampling.json`; local artifacts:
`experiments/t20-bench/target/native-d264-singleton.n8JR0V`.
...
and 40 groups with output digest
`aec16fdc8a1630a97eace654862a192929550c5fb771aed0862a9bed67d81584`. After removing only timing,
percentile and RSS fields, the complete report exactly matches D0261 at SHA-256
`c472dc13e72e3ed3fd5cd46776824223c89213f9c2dc9ec25bdcc653b633abd9`; all semantic and physical
work evidence is unchanged.

The range high-water remained 29,231,596 bytes with zero evictions/evicted bytes. The retained half
hit all 1,074,437 ranges with zero misses and zero adapter reads; empty reads remained 11,240,354.
The round took 544,273 ms, 0.24% below D0261. Retained all-class successful p99 was **2.766824 /
91.255523 / 63.166153 / 381.871066 ms**, changing by +3.16%, -0.23%, -1.34% and -1.25%. This single
sequential uncontrolled observation cannot establish causality; four-hop remains 1.53 times the
unchanged 250 ms target, so no target, T-20, M1 or qualification gate passes.

Process peak RSS was 265,544 KiB; wall/user/system were 720.79/664.45/54.65 seconds with zero
process swaps. Inputs and the D0262 binary stayed pinned. Shared memory/swap peaks were unchanged;
soft-limit events increased by 22 while maximum/OOM/socket-memory/CPU-throttle counters did not
increase. Complete report: `docs/evidence/fixed-stored-record-field-table-sampling.json`; local
artifacts: `experiments/t20-bench/target/native-d262-fields.RIbtaG`.
...
and 40 groups with output digest
`aec16fdc8a1630a97eace654862a192929550c5fb771aed0862a9bed67d81584`. After removing only timing,
percentile and RSS fields, the complete report exactly matches D0255 at SHA-256
`c472dc13e72e3ed3fd5cd46776824223c89213f9c2dc9ec25bdcc653b633abd9`; all semantic and physical
work evidence is unchanged.

The range high-water remained 29,231,596 bytes with zero evictions/evicted bytes. The retained half
hit all 1,074,437 ranges with zero misses and zero adapter reads; empty reads remained 11,240,354.
The round took 541,442 ms, 0.01% above D0255. Retained all-class successful p99 was **2.660155 /
87.684055 / 60.252556 / 368.605719 ms**, changing by +2.17%, +1.28%, -0.02% and +1.40%. This
single sequential uncontrolled observation cannot establish causality; four-hop remains 1.47 times
the unchanged 250 ms target, so no target, T-20, M1 or qualification gate passes.

Process peak RSS was 265,984 KiB; wall/user/system were 719.17/662.81/53.74 seconds with zero
process swaps. Inputs and the D0256 binary stayed pinned. Shared memory/swap peaks were unchanged;
soft-limit events increased by 1,213 while maximum/OOM/CPU-throttle counters remained zero.
Complete report: `docs/evidence/borrowed-direct-string-sampling.json`; local artifacts:
`experiments/t20-bench/target/native-d256-direct.Ene44N`.

Next commit/push this evidence, then inspect nested fixed-schema map construction in repeated
stored-record decoding or select another measured zero-I/O retained-path cost. Preserve existing
...
and 40 groups with output digest
`aec16fdc8a1630a97eace654862a192929550c5fb771aed0862a9bed67d81584`. After removing only timing,
percentile and RSS fields, the complete report exactly matches D0253 at SHA-256
`c472dc13e72e3ed3fd5cd46776824223c89213f9c2dc9ec25bdcc653b633abd9`; all semantic and physical
work evidence is unchanged.

The range high-water remained 29,231,596 bytes with zero evictions/evicted bytes. The retained half
hit all 1,074,437 ranges with zero misses and zero adapter reads; empty reads remained 11,240,354.
The round took 541,403 ms, 5.15% below D0253. Retained all-class successful p99 was **2.603691 /
86.577961 / 60.266886 / 363.516682 ms**, changing by -17.49%, -17.24%, -15.44% and -13.83%. This
single sequential uncontrolled observation cannot establish causality; four-hop remains 1.45 times
the unchanged 250 ms target, so no target, T-20, M1 or qualification gate passes.

Process peak RSS was 265,992 KiB; wall/user/system were 719.52/663.21/53.70 seconds with zero
process swaps. Inputs and the D0254 binary stayed pinned. Shared memory/swap peaks were unchanged;
soft-limit events increased by 1,679 while maximum/OOM/CPU-throttle counters remained zero.
Complete report: `docs/evidence/borrowed-schema-key-sampling.json`; local artifacts:
`experiments/t20-bench/target/native-d254-borrowed.WlI5G7`.

Next commit/push this evidence, then inspect remaining owned scalar/string construction in repeated
stored-record decoding or select another measured zero-I/O retained-path cost. Preserve existing
defaults and all qualification prerequisites.
...
and 40 groups with output digest
`aec16fdc8a1630a97eace654862a192929550c5fb771aed0862a9bed67d81584`. After removing only timing,
percentile and RSS fields, the complete report exactly matches D0249 at SHA-256
`c472dc13e72e3ed3fd5cd46776824223c89213f9c2dc9ec25bdcc653b633abd9`; all semantic and physical
work evidence is unchanged.

The range high-water remained 29,231,596 bytes with zero evictions/evicted bytes. The retained half
hit all 1,074,437 ranges with zero misses and zero adapter reads; empty reads remained 11,240,354.
The round took 607,136 ms, 2.01% above D0249. Retained all-class successful p99 was **4.129358 /
129.888535 / 93.914371 / 522.109698 ms**, changing by +4.94%, +8.11%, +10.81% and +9.74%. This
single sequential uncontrolled observation cannot establish causality; four-hop remains 2.09 times
the unchanged 250 ms target, so no target, T-20, M1 or qualification gate passes.

Process peak RSS was 265,684 KiB; wall/user/system were 795.56/737.94/55.81 seconds with zero
process swaps. Inputs and the D0250 binary stayed pinned. Shared memory/swap peaks were unchanged;
soft-limit events increased by 965 while maximum/OOM/CPU-throttle counters remained zero. Complete
report: `docs/evidence/fallible-range-mapping-sampling.json`; accepted local artifacts:
`experiments/t20-bench/target/native-d250-fallible.6rs555`. A retained 0.41-second wrong-root
preflight produced no report and is not an observation.

Next commit/push this evidence, then inspect repeated decoded-record construction on the zero-I/O
retained path. Preserve existing defaults and all qualification prerequisites.
...
and 40 groups with output digest
`aec16fdc8a1630a97eace654862a192929550c5fb771aed0862a9bed67d81584`. After removing only timing,
percentile and RSS fields, the complete report exactly matches D0247 at SHA-256
`c472dc13e72e3ed3fd5cd46776824223c89213f9c2dc9ec25bdcc653b633abd9`; all semantic and physical
work evidence is unchanged.

The range high-water remained 29,231,596 bytes with zero evictions/evicted bytes. The retained half
hit all 1,074,437 ranges with zero misses and zero adapter reads; empty reads remained 11,240,354.
The round took 595,191 ms, 2.13% above D0247. Retained all-class successful p99 was **3.935008 /
120.144452 / 84.756031 / 475.754743 ms**, changing by -0.13%, -5.00%, -5.53% and -2.88%. This
single sequential uncontrolled observation cannot establish causality; four-hop remains 1.90 times
the unchanged 250 ms target, so no target, T-20, M1 or qualification gate passes.

Process peak RSS was 265,856 KiB; wall/user/system were 790.64/732.08/55.71 seconds with zero
process swaps. Inputs and the D0248 binary stayed pinned. Shared memory/swap peaks were unchanged;
soft-limit events increased by 1,434 while maximum/OOM/CPU-throttle counters remained zero.
Complete report: `docs/evidence/borrowed-authorization-sampling.json`; local artifacts:
`experiments/t20-bench/target/native-d248-authz.TXqOjg`.

Next commit/push this evidence, then inspect repeated decoded-record and compact result-vector
construction on the zero-I/O retained path. Preserve existing defaults and all qualification
prerequisites.
