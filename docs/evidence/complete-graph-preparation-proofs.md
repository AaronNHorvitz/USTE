...
Decision 0037 subsequently derives a terminal-root delta from this proof result, and Decision 0038
binds it to the authoritative coordinator commit. The coordinator's live publication target and
postcommit admitted-root comparison still retain the complete graph. Prefix collection is capped
but not streaming reducer evaluation, per-prefix index results remain capped at 64 MiB, and no
