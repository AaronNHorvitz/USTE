...
- **World creation is durable-or-absent.** Creation synchronously publishes, in order, the manifest, the empty log, and the initial frontier record (`F` = log start, both slots written and fsynced) — and only then reports success. A crash mid-creation leaves a partial world directory that never reported success: it is deleted and creation retried, never recovered. Corollary: **every legal world has a valid frontier from birth**, so dual-slot frontier loss during recovery always indicates corruption and never a fresh world — which is what makes the hard-error rule below sound.
